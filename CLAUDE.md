...
APT_insight_04/
├── pipeline/
│   ├── collect_rt.py              # 국토부 36개월 초기 수집·가공
│   ├── collect_naver.py           # 네이버 매물 초기 전체 수집 (full)
│   ├── naver_session.py           # 네이버 API 세션 + adaptive delay (교재 10장)
│   ├── schemas.py                 # 국토부 스키마 변환 공통 (convert_to_*, extract_complex_info)
│   ├── update_rt_daily.py         # 일일 국토부 갱신 + 36개월 cleanup (교재 17장)
│   ├── update_nv_daily.py         # 일일 네이버 갱신 + diff_listings 순수함수 (교재 18장)
│   ├── build_mapping.py           # rt_complex ↔ nv_complex 매핑 (교재 14장)
│   ├── run_daily.py               # 일일 파이프라인 오케스트레이터 + JSON 리포트 (교재 19장)
│   └── utils.py                   # 공통 유틸리티 (API, CSV, Kakao 지오코딩)
├── agents/
...
| 시각 (KST) | 명령 |
|------------|------|
| 03:00 | `python -m pipeline.run_daily` (RT → NV 순차, JSON 리포트 `data/reports/YYYY-MM-DD.json`) |

매핑 재실행이 필요할 때: `python -m pipeline.build_mapping` (수동).
