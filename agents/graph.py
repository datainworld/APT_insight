...
- **시간 범위 기본값** (sql_query): 기간이 명시되지 않은 rt_trade/rt_rent 쿼리는 기본 "최근 6개월" 로 제한. "최근/요즘/지금" 키워드면 "최근 3개월", "올해/이번" 이면 "최근 12개월", "3년간/장기 추이" 등 명시적 장기 질의만 36개월 전체 허용. 네이버 nv_listing 기본 조건은 `is_active = TRUE` (현재 시점 호가).
"""
...
def synthesize(state: SupervisorState) -> dict:
    ...
    prompt = SystemMessage(content=(
        ...
        "## 소스별 시점 (Recency) 규칙\n"
        f"- 각 소스의 데이터 시점을 답변에 명확히 표시하세요 (예: '(국토부 YYYY-MM) / (뉴스 YYYY-MM-DD 보도) / (PDF 업로드 시점 기준)').\n"
        f"- 사용자가 '최근/지금/요즘' 을 강조하면 가장 최신 소스를 중심으로 답하세요. DB 는 오늘({today_str}) 기준 직전 1~3개월, 뉴스는 60일 내, PDF 는 최신성 검증 불가 → 보조 근거로만.\n"
        "- 소스 간 불일치 시 최신 시점을 우선하되 불일치 자체를 숨기지 말고 '시차로 갱신 안 된 가능성' 으로 언급하세요.\n\n"
        "## 출처 표시 (중요)\n"
